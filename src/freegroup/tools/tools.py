...
def _negative_generators(fdim):
    return [-x for x in range(1, fdim + 1)]

def _positive_generators(fdim):
    return [x for x in range(1, fdim + 1)]

def generators(fdim, t = "all"):
    if t == "all":
        return _negative_generators(fdim) + _positive_generators(fdim)
    if t == "positive":
        return _positive_generators(fdim)
    if t == "negative":
        return _negative_generators(fdim)
...
def permute_generators(word, permutation = "cycle_shift", fdim = None, **kwargs):
    ...
    if not isinstance(permutation, str):
        raise ValueError(f'Type of `permutation` should be either `str` or `dict`. Got {type(permutation)}')
        
    if permutation == 'cycle_shift':
        permutation, shift = {}, kwargs.get('shift', 1)
        
        for x in generators(fdim, t = "negative"):
            permutation[x] = -( (abs(x) - 1 + shift) % fdim + 1 )
